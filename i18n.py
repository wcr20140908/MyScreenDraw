...
_BASE = {
    ...
    "update_open_page": ("是否打开下载页面？程序不会自动下载或安装任何文件。",
                         "Open the download page? Nothing is downloaded or installed automatically.",
                         "Ouvrir la page de téléchargement ? Rien n'est téléchargé ni installé automatiquement.",
                         "¿Abrir la página de descarga? Nada se descarga ni instala automáticamente.",
                         "Download-Seite öffnen? Es wird nichts automatisch heruntergeladen oder installiert.",
                         "Открыть страницу загрузки? Ничего не скачивается и не устанавливается автоматически.",
                         "다운로드 페이지를 열까요? 자동으로 내려받거나 설치하지 않습니다.",
                         "ダウンロードページを開きますか？自動的に取得・インストールはしません。"),
    ...
    "update_hint": ("默认关闭。开启后也只在你点「立即检查」时联网一次，只读取版本号，不上传任何数据。",
                    "Off by default. When on, it only contacts the server when you tap Check now, reads the version number, and uploads nothing.",
                    "Désactivé par défaut. Une fois activé, la connexion n'a lieu qu'au clic sur « Vérifier », lit le numéro de version et n'envoie rien.",
                    "Desactivado por defecto. Al activarlo, solo conecta cuando pulsa Buscar, lee el número de versión y no envía nada.",
                    "Standardmäßig aus. Aktiviert wird nur bei „Jetzt prüfen“ verbunden, die Versionsnummer gelesen und nichts gesendet.",
                    "По умолчанию выключено. При включении соединение только по нажатию «Проверить», читается лишь номер версии, ничего не отправляется.",
                    "기본적으로 꺼져 있습니다. 켜도 «지금 확인»을 누를 때만 접속해 버전 번호만 읽고 아무것도 보내지 않습니다.",
                    "既定はオフ。オンにしても「今すぐ確認」を押した時だけ通信し、バージョン番号のみ読み取り、何も送信しません。"),

...
_BASE = {
    ...
    # 两者必须用不同的词，否则用户点了「关闭」发现进程还在，会以为程序没退干净。
    "close_software": ("关闭软件", "Close", "Fermer", "Cerrar", "Schließen", "Закрыть", "닫기", "閉じる"),
    "logo_tip": ("单击折叠/展开，按住拖动", "Click to fold, drag to move", "Clic : replier ; glisser : déplacer", "Clic para plegar, arrastrar para mover", "Klicken zum Einklappen, ziehen zum Verschieben", "Клик — свернуть, перетащить — переместить", "클릭하여 접기, 끌어서 이동", "クリックで折りたたみ、ドラッグで移動"),
    "show_main_ui": ("显示主界面", "Show window", "Afficher", "Mostrar ventana", "Fenster zeigen", "Показать окно", "창 표시", "ウィンドウ表示"),
    "hide_main_ui": ("隐藏主界面", "Hide window", "Masquer", "Ocultar ventana", "Fenster ausblenden", "Скрыть окно", "창 숨기기", "ウィンドウ非表示"),
    ...
    "hide_main_ui": ("隐藏主界面", "Hide Main UI", "Masquer l'interface", "Ocultar interfaz", "Hauptoberfläche ausblenden", "Скрыть интерфейс", "메인 UI 숨기기", "メインUIを非表示"),
    "exit_title": ("确认退出", "Confirm Exit", "Confirmer la sortie", "Confirmar salida", "Beenden bestätigen", "Подтвердить выход", "종료 확인", "終了確認"),
    "exit_prompt": ("是否保存当前项目？", "Save current project?", "Sauvegarder le projet actuel?", "¿Guardar proyecto actual?", "Aktuelles Projekt speichern?", "Сохранить текущий проект?", "현재 프로젝트를 저장하시겠습니까?", "現在のプロジェクトを保存しますか？"),
    "exit_cancel": ("不再退出", "Cancel", "Annuler", "Cancelar", "Abbrechen", "Отмена", "취소", "キャンセル"),
    "exit_no_save": ("直接退出", "Quit Without Saving", "Quitter sans sauvegarder", "Salir sin guardar", "Ohne Speichern beenden", "Выйти без сохранения", "저장하지 않고 종료", "保存せずに終了"),
    "exit_save": ("保存并退出", "Save and Quit", "Sauvegarder et quitter", "Guardar y salir", "Speichern und beenden", "Сохранить и выйти", "저장하고 종료", "保存して終了"),
    "quit_app": ("退出软件", "Quit", "Quitter", "Salir", "Beenden", "Выход", "종료", "終了"),  # 别名
    # ---------- 退出确认 ----------
    "exit_title": ("退出软件", "Quit", "Quitter", "Salir", "Beenden", "Выход", "종료", "終了"),
    "exit_prompt": ("是否保存当前项目？", "Save the current project?", "Enregistrer le projet ?", "¿Guardar el proyecto?", "Projekt speichern?", "Сохранить проект?", "현재 프로젝트를 저장할까요?", "現在のプロジェクトを保存しますか？"),
    "exit_cancel": ("不再退出", "Don't quit", "Ne pas quitter", "No salir", "Nicht beenden", "Не выходить", "종료 취소", "終了しない"),
    "exit_no_save": ("直接退出", "Quit without saving", "Quitter sans enregistrer", "Salir sin guardar", "Ohne Speichern beenden", "Выйти без сохранения", "저장 없이 종료", "保存せず終了"),
    "exit_save": ("保存退出", "Save and quit", "Enregistrer et quitter", "Guardar y salir", "Speichern und beenden", "Сохранить и выйти", "저장 후 종료", "保存して終了"),
    "restart_failed": ("重启失败，已保留当前窗口。", "Restart failed; the current window was kept.", "Échec du redémarrage ; la fenêtre actuelle a été conservée.", "No se pudo reiniciar; se mantuvo la ventana actual.", "Neustart fehlgeschlagen; das aktuelle Fenster bleibt geöffnet.", "Не удалось перезапустить; текущее окно сохранено.", "다시 시작하지 못했습니다. 현재 창을 유지했습니다.", "再起動できませんでした。現在のウィンドウを維持します。"),
    ...
    "page_list": ("页面列表", "Pages", "Pages", "Páginas", "Seiten", "Страницы", "페이지 목록", "ページ一覧"),
    "page_of": ("页面 {value}", "Page {value}", "Page {value}", "Página {value}", "Seite {value}", "Страница {value}", "페이지 {value}", "ページ {value}"),
    ...
    # ---------- 设置页（5.5.0）----------
    "settings": ("设置", "Settings", "Réglages", "Ajustes", "Einstellungen", "Настройки", "설정", "設定"),
    "settings_appearance": ("外观", "Appearance", "Apparence", "Apariencia", "Darstellung", "Внешний вид", "모양", "外観"),
    "settings_panel_group": ("主面板", "Main panel", "Panneau principal", "Panel principal", "Hauptleiste", "Главная панель", "메인 패널", "メインパネル"),
    "settings_drawing": ("绘制", "Drawing", "Dessin", "Dibujo", "Zeichnen", "Рисование", "그리기", "描画"),
    "settings_system": ("系统", "System", "Système", "Sistema", "System", "Система", "시스템", "システム"),
    "settings_about": ("关于", "About", "À propos", "Acerca de", "Über", "О программе", "정보", "情報"),
    "about_version": ("MyScreenDraw {value}\n开源软件，GPL-3.0 许可证",
                      "MyScreenDraw {value}\nOpen source, GPL-3.0 licence",
                      "MyScreenDraw {value}\nLogiciel libre, licence GPL-3.0",
                      "MyScreenDraw {value}\nCódigo abierto, licencia GPL-3.0",
                      "MyScreenDraw {value}\nOpen Source, GPL-3.0-Lizenz",
                      "MyScreenDraw {value}\nОткрытый код, лицензия GPL-3.0",
                      "MyScreenDraw {value}\n오픈 소스, GPL-3.0 라이선스",
                      "MyScreenDraw {value}\nオープンソース、GPL-3.0 ライセンス"),
    "orientation": ("工具栏方向", "Toolbar direction", "Sens de la barre", "Dirección de barra", "Leistenrichtung", "Направление панели", "도구 모음 방향", "ツールバー方向"),
    "orient_portrait": ("竖版", "Vertical", "Vertical", "Vertical", "Vertikal", "Вертикально", "세로", "縦型"),
    "orient_landscape": ("横版", "Horizontal", "Horizontal", "Horizontal", "Horizontal", "Горизонтально", "가로", "横型"),
    "radius_value": ("圆角: {value}", "Corner radius: {value}", "Arrondi : {value}", "Redondeo: {value}", "Eckenradius: {value}", "Скругление: {value}", "모서리: {value}", "角丸: {value}"),
    "opacity_hint": ("只影响操作界面，不影响画好的墨迹",
                     "Affects the panels only, never your ink",
                     "N'affecte que les panneaux, jamais votre encre",
                     "Solo afecta a los paneles, nunca a su tinta",
                     "Betrifft nur die Bedienleisten, nie Ihre Zeichnung",
                     "Влияет только на панели, не на ваши чернила",
                     "패널에만 적용되며 필기 내용은 그대로입니다",
                     "パネルのみに適用され、書いた線には影響しません"),
    "multitouch_on": ("多指同时书写：开", "Multi-touch writing: ON", "Écriture multi-doigts : ON", "Escritura multitáctil: ON", "Mehrfinger-Schreiben: EIN", "Многопальцевое письмо: ВКЛ", "멀티터치 필기: 켜짐", "マルチタッチ筆記: オン"),
    "multitouch_off": ("多指同时书写：关", "Multi-touch writing: OFF", "Écriture multi-doigts : OFF", "Escritura multitáctil: OFF", "Mehrfinger-Schreiben: AUS", "Многопальцевое письмо: ВЫКЛ", "멀티터치 필기: 꺼짐", "マルチタッチ筆記: オフ"),
    "speed_width_on": ("笔速影响粗细：开", "Speed-based width: ON", "Épaisseur selon vitesse : ON", "Grosor por velocidad: ON", "Geschwindigkeitsstärke: EIN", "Толщина от скорости: ВКЛ", "속도 기반 굵기: 켜짐", "速度で太さ変化: オン"),
    "speed_width_off": ("笔速影响粗细：关", "Speed-based width: OFF", "Épaisseur selon vitesse : OFF", "Grosor por velocidad: OFF", "Geschwindigkeitsstärke: AUS", "Толщина от скорости: ВЫКЛ", "속도 기반 굵기: 꺼짐", "速度で太さ変化: オフ"),

    # ---------- 开机自启 ----------
    "autostart_on": ("开机自动启动：开", "Start with Windows: ON", "Démarrer avec Windows : ON", "Iniciar con Windows: ON", "Mit Windows starten: EIN", "Запуск с Windows: ВКЛ", "Windows 시작 시 실행: 켜짐", "Windows 起動時に実行: オン"),
    "autostart_off": ("开机自动启动：关", "Start with Windows: OFF", "Démarrer avec Windows : OFF", "Iniciar con Windows: OFF", "Mit Windows starten: AUS", "Запуск с Windows: ВЫКЛ", "Windows 시작 시 실행: 꺼짐", "Windows 起動時に実行: オフ"),
    "autostart_hint": ("写入当前用户的启动项（注册表 HKCU\\...\\Run），不影响其他用户",
                       "Adds a Run entry for the current user only (HKCU\\...\\Run)",
                       "Ajoute une entrée Run pour l'utilisateur courant uniquement (HKCU\\...\\Run)",
                       "Añade una entrada Run solo para el usuario actual (HKCU\\...\\Run)",
                       "Legt einen Run-Eintrag nur für den aktuellen Benutzer an (HKCU\\...\\Run)",
                       "Добавляет запись Run только для текущего пользователя (HKCU\\...\\Run)",
                       "현재 사용자에게만 Run 항목을 추가합니다 (HKCU\\...\\Run)",
                       "現在のユーザーのみに Run 項目を追加します (HKCU\\...\\Run)"),
    "autostart_saved": ("已设置，下次开机自动启动", "Saved. It will start with Windows.", "Enregistré. Démarrera avec Windows.", "Guardado. Se iniciará con Windows.", "Gespeichert. Startet mit Windows.", "Сохранено. Запустится вместе с Windows.", "저장되었습니다. 다음 부팅 시 실행됩니다.", "保存しました。次回起動時に自動実行されます。"),
    "autostart_failed": ("设置失败：注册表可能被管理员策略锁定",
                         "Could not save: the registry may be locked by policy",
                         "Échec : le registre est peut-être verrouillé par une stratégie",
                         "No se pudo guardar: el registro puede estar bloqueado por directiva",
                         "Fehlgeschlagen: Registry ist möglicherweise per Richtlinie gesperrt",
                         "Не удалось сохранить: реестр может быть заблокирован политикой",
                         "저장 실패: 정책으로 레지스트리가 잠겨 있을 수 있습니다",
                         "保存できません: ポリシーでレジストリがロックされている可能性があります"),

    # ---------- 检查更新（默认关闭）----------
    "update_channel_stable": ("正式版", "Stable release", "Version stable", "Versión estable", "Stabile Version", "Стабильная версия", "안정 버전", "安定版"),
    "update_channel_preview": ("预览版", "Preview release", "Version preview", "Versión preliminar", "Vorschauversion", "Предварительная версия", "미리보기 버전", "プレビュー版"),
    "update_download": ("下载更新", "Download update", "Télécharger la mise à jour", "Descargar actualización", "Update herunterladen", "Скачать обновление", "업데이트 다운로드", "更新をダウンロード"),
    "update_install_prompt": ("下载完成。现在重启应用以安装吗？设置和用户数据不会被覆盖。", "Download complete. Restart now to install? Settings and user data will be preserved.", "Téléchargement terminé. Redémarrer pour installer ? Les réglages et données seront conservés.", "Descarga completa. ¿Reiniciar para instalar? Se conservarán los ajustes y datos.", "Download abgeschlossen. Jetzt zur Installation neu starten? Einstellungen und Daten bleiben erhalten.", "Загрузка завершена. Перезапустить для установки? Настройки и данные сохранятся.", "다운로드 완료. 설치를 위해 지금 다시 시작할까요? 설정과 사용자 데이터는 보존됩니다.", "ダウンロード完了。インストールのため再起動しますか？設定とユーザーデータは保持されます。"),
    "update_channel_hint": ("选择正式版或预览版；仅在你点击检查时联网。", "Choose stable or preview releases; network access occurs only when you check.", "Choisissez stable ou preview ; le réseau n'est utilisé qu'à la vérification.", "Elige versiones estables o preliminares; solo se conecta al comprobar.", "Wählen Sie stabile oder Vorschauversionen; Verbindung nur beim Prüfen.", "Выберите стабильные или предварительные версии; сеть используется только при проверке.", "안정 또는 미리보기 버전을 선택하며 확인할 때만 연결합니다.", "安定版またはプレビュー版を選択し、確認時のみ通信します。"),
    "update_check_on": ("允许检查更新：开", "Allow update checks: ON", "Vérification des MAJ : ON", "Buscar actualizaciones: ON", "Updateprüfung: EIN", "Проверка обновлений: ВКЛ", "업데이트 확인 허용: 켜짐", "更新確認を許可: オン"),
    "update_check_off": ("允许检查更新：关", "Allow update checks: OFF", "Vérification des MAJ : OFF", "Buscar actualizaciones: OFF", "Updateprüfung: AUS", "Проверка обновлений: ВЫКЛ", "업데이트 확인 허용: 꺼짐", "更新確認を許可: オフ"),
    "check_update_now": ("立即检查更新", "Check now", "Vérifier maintenant", "Buscar ahora", "Jetzt prüfen", "Проверить сейчас", "지금 확인", "今すぐ確認"),
    "update_checking": ("正在检查…", "Checking…", "Vérification…", "Buscando…", "Prüfe…", "Проверка…", "확인 중…", "確認中…"),
    "update_current": ("已是最新版本", "You are up to date", "Vous êtes à jour", "Está actualizado", "Sie sind aktuell", "У вас последняя версия", "최신 버전입니다", "最新版です"),
    "update_available": ("发现新版本 {value}", "New version {value} available", "Nouvelle version {value}", "Nueva versión {value}", "Neue Version {value}", "Доступна версия {value}", "새 버전 {value}", "新しいバージョン {value}"),
    "update_open_page": ("准备下载新版本 {value}。确认后将在应用内下载，安装时保留设置和用户数据。", "Download version {value} inside the app? Settings and user data will be preserved during installation.", "Télécharger la version {value} dans l'application ? Les réglages et données seront conservés.", "¿Descargar la versión {value} dentro de la aplicación? Se conservarán ajustes y datos.", "Version {value} in der Anwendung herunterladen? Einstellungen und Daten bleiben erhalten.", "Скачать версию {value} внутри приложения? Настройки и данные будут сохранены.", "앱 안에서 버전 {value}을(를) 다운로드할까요? 설정과 사용자 데이터는 보존됩니다.", "バージョン {value}をアプリ内でダウンロードしますか？設定とユーザーデータは保持されます。"),
    "update_failed": ("检查失败：{detail}", "Check failed: {detail}", "Échec : {detail}", "Error: {detail}", "Fehlgeschlagen: {detail}", "Ошибка: {detail}", "확인 실패: {detail}", "確認失敗: {detail}"),
    "update_rate_limited": ("查询次数超限，过一会儿再试（同一网络出口共用配额）",
                            "Too many checks from this network; try again later (the quota is shared per network)",
                            "Trop de vérifications depuis ce réseau ; réessayez plus tard (quota partagé par réseau)",
                            "Demasiadas consultas desde esta red; inténtelo más tarde (la cuota se comparte por red)",
                            "Zu viele Abfragen aus diesem Netzwerk; später erneut versuchen (Kontingent pro Netzwerk)",
                            "Слишком много запросов из этой сети; повторите позже (квота общая для сети)",
                            "이 네트워크에서 확인 요청이 너무 많습니다. 잠시 후 다시 시도하세요(할당량은 네트워크 단위로 공유됩니다)",
                            "このネットワークからの確認が多すぎます。しばらくしてから再試行してください（割り当ては回線単位で共有されます）"),
    "update_hint": ("默认关闭。开启后仅在你点击检查时联网；可选择正式版或预览版，并在应用内确认下载与安装。", "Off by default. When enabled, it connects only when you check; choose stable or preview releases and confirm downloads and installation inside the app.", "Désactivé par défaut. La connexion n'a lieu qu'à la vérification ; choisissez stable ou preview et confirmez le téléchargement et l'installation dans l'application.", "Desactivado por defecto. Solo conecta al comprobar; elige versiones estables o preliminares y confirma la descarga y la instalación dentro de la aplicación.", "Standardmäßig aus. Verbindung nur beim Prüfen; stabile oder Vorschauversion wählen und Download sowie Installation in der Anwendung bestätigen.", "По умолчанию выключено. Сеть используется только при проверке; выберите стабильную или предварительную версию и подтвердите загрузку и установку в приложении.", "기본적으로 꺼져 있습니다. 확인할 때만 연결하며 안정/미리보기 버전을 선택하고 앱 안에서 다운로드와 설치를 확인합니다.", "既定はオフ。確認時のみ通信し、安定版またはプレビュー版を選び、アプリ内でダウンロードとインストールを確認します。"),
    ...
    "text_editor_title": ("文字 / 公式", "Text / formula", "Texte / formule", "Texto / fórmula",
                          "Text / Formel", "Текст / формула", "텍스트 / 수식", "テキスト / 数式"),
    "text_done": ("完成", "Done", "Terminé", "Listo", "Fertig", "Готово", "완료", "完了"),
    "text_newline": ("换行", "New line", "Nouvelle ligne", "Nueva línea",
                     "Neue Zeile", "Новая строка", "줄 바꿈", "改行"),
    "text_backspace": ("退格", "Backspace", "Retour", "Retroceso",
                       "Rücktaste", "Забой", "백스페이스", "後退"),
    "text_keyboard": ("键盘", "Keyboard", "Clavier", "Teclado",
                      "Tastatur", "Клавиатура", "키보드", "キーボード"),
    "text_keyboard_missing": ("系统触摸键盘不可用", "System touch keyboard unavailable",
                              "Clavier tactile système indisponible",
                              "Teclado táctil del sistema no disponible",
                              "System-Touch-Tastatur nicht verfügbar",
                              "Экранная клавиатура недоступна",
                              "시스템 터치 키보드를 사용할 수 없습니다",
                              "システムタッチキーボードは利用できません"),
    "text_keyboard_no_touch": ("本机无触摸屏，系统不弹出触摸键盘；已尝试屏幕键盘",
                               "No touch screen: Windows withholds the touch keyboard. Tried the On-Screen Keyboard instead.",
                               "Pas d'écran tactile : Windows retient le clavier tactile. Clavier visuel essayé à la place.",
                               "Sin pantalla táctil: Windows retiene el teclado táctil. Se intentó el teclado en pantalla.",
                               "Kein Touchscreen: Windows hält die Touch-Tastatur zurück. Bildschirmtastatur versucht.",
                               "Нет сенсорного экрана: Windows не показывает сенсорную клавиатуру. Использована экранная.",
                               "터치 스크린이 없어 Windows가 터치 키보드를 표시하지 않습니다. 화상 키보드를 시도했습니다.",
                               "タッチスクリーンがないため、Windows はタッチキーボードを表示しません。スクリーンキーボードを試しました。"),
    "sym_group_greek": ("希腊字母", "Greek", "Grec", "Griego", "Griechisch",
                        "Греческие", "그리스 문자", "ギリシャ文字"),
    "sym_group_operator": ("运算符", "Operators", "Opérateurs", "Operadores",
                           "Operatoren", "Операторы", "연산자", "演算子"),
    "sym_group_relation": ("集合与逻辑", "Sets & logic", "Ensembles et logique",
                           "Conjuntos y lógica", "Mengen & Logik",
                           "Множества и логика", "집합과 논리", "集合と論理"),
    "sym_group_calculus": ("微积分", "Calculus", "Analyse", "Cálculo",
                           "Analysis", "Анализ", "미적분", "微積分"),
    "sym_group_structure": ("公式结构", "Structures", "Structures", "Estructuras",
                            "Strukturen", "Структуры", "구조", "構造"),
